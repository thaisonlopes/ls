...
@WebFilter(filterName = "paginaRestritaFilter", urlPatterns = { "/nexus/ged/*" })
				...
				if (loginUsuario.isLogado()) {
					...
					if (u.getTipoAcesso().getCodigo() < 20) {
						...
						if (u != null) {
							...
							int tipoPermicao = u.getTipoPermissao().getCodigo();
							if (tipoPermicao < 5) {
								long idUE = u.getCodigo();
	...
	public void doFilter(ServletRequest request, ServletResponse response, FilterChain chain)
		...
		try {
			...
			if (redirecionar) {
				...
				HttpServletResponse httpResponse = (HttpServletResponse) response;
				String url = httpResponse.encodeRedirectURL(contextPath + "/nexus/access.xhtml");
				httpResponse.sendRedirect(url);
				return;
